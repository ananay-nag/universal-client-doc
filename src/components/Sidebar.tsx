...
export default function Sidebar({ apis, selectedId, onSelect }: SidebarProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const classes = (apis.claasNames || [])
      .map(cls => {
        const constructors = (cls.constructor || []).filter(c =>
          (`${cls.className}()`).toLowerCase().includes(term) ||
          ((Array.isArray(c.description) ? c.description.join(' ') : c.description || '') as string)
            .toLowerCase()
            .includes(term)
        );
  ...
  return (
    <aside className="w-full max-w-xs md:max-w-sm bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col">
      ...
      {/* List */}
      <nav className="flex-1 overflow-y-auto p-4 space-y-2" aria-label="API List">
        {/* Classes */}
          ...
          <div>
            {filtered.classes.map(cls => (
              <div key={cls.id} className="mb-1">
                ...
                >
                  Classes
                  {openClasses[cls.id] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                </button>
                {openClasses[cls.id] && (
                  <div className="pl-4 py-2 space-y-1">
                    <h2 className="font-bold mb-1 text-blue-800 dark:text-white">Construtors</h2>
                    ...
                    <h2 className="font-bold mb-1 text-blue-800 dark:text-white">Methods</h2>
                    {cls.methods.map(m => (
                      <div
                        key={m.methodId}
        ...
        {filtered.methods.length > 0 && (
          <div>
            ...
            >
              Methods
              {methodsOpen ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
            </button>
            {methodsOpen && (
              <div className="pl-4 py-2 space-y-1">
        ...
        {filtered.classes.length === 0 && filtered.methods.length === 0 && (
          <p className="text-gray-500 dark:text-gray-400 text-center mt-4">No APIs found.</p>
        )}
      </nav>
    </aside>
  );
